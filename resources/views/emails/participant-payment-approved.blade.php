...
<head>
    ...
    <title>Pendaftaran disetujui, lanjutkan pembayaran</title>
</head>
                ...
                <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="max-width:680px; background-color:#ffffff; border-radius:18px; overflow:hidden;">
                    <tr>
                        <td style="background:linear-gradient(135deg, #0f766e 0%, #134e4a 100%); padding:30px 24px; text-align:center; color:#ffffff;">
                            ...
                            <div style="margin-top:8px; font-size:15px; opacity:0.92;">Pendaftaran Anda disetujui</div>
                        </td>
                    </tr>
                    <tr>
                        <td style="padding:28px 24px 12px;">
                            <span style="display:inline-block; padding:8px 14px; border-radius:999px; background-color:#ecfdf5; color:#047857; font-size:12px; font-weight:700; letter-spacing:0.08em;">SIAP PEMBAYARAN</span>
                            <p style="margin:20px 0 12px; font-size:16px; line-height:1.7;">Halo {{ $participant->name }},</p>
                            <p style="margin:0 0 12px; font-size:16px; line-height:1.7;">Pendaftaran Anda untuk <strong>{{ $participant->event?->name ?? 'Samawa Run' }}</strong> kategori <strong>{{ $participant->distance_category }}</strong> telah disetujui dan siap masuk ke tahap pembayaran.</p>
                            <p style="margin:0 0 12px; font-size:16px; line-height:1.7;">Silakan transfer sesuai nominal berikut, lalu upload bukti pembayaran melalui tombol di bawah email ini.</p>
                        </td>
                    </tr>
                    <tr>
                        <td style="padding:0 24px;">
                            <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="border:1px solid #d1fae5; border-radius:18px; overflow:hidden; background:linear-gradient(180deg, #f0fdf4 0%, #ecfeff 100%);">
                                <tr>
                                    <td style="padding:22px 20px; text-align:center;">
                                        <div style="font-size:11px; font-weight:700; letter-spacing:0.16em; color:#047857; text-transform:uppercase;">Nominal yang harus dibayar</div>
                                        <div style="margin-top:8px; font-size:34px; font-weight:800; color:#0f172a;">{{ $paymentAmount }}</div>
                                        <div style="margin-top:8px; font-size:13px; color:#475569;">Mohon transfer sesuai nominal kategori agar verifikasi lebih cepat.</div>
                                    </td>
                                </tr>
                            </table>
                        </td>
                    ...
                    <tr>
                        <td style="padding:0 24px 24px;">
                            <div style="padding:16px 18px; border-radius:14px; background-color:#f8fafc; font-size:14px; line-height:1.7; color:#475569;">
                                ...
                                <div>1. Upload bukti transfer dalam format JPG, PNG, atau PDF.</div>
                                <div>2. Pastikan nominal transfer sesuai dengan kategori yang dipilih.</div>
                                <div>3. Setelah bukti diupload, admin akan melakukan review kembali.</div>
                                @if ($contact)
